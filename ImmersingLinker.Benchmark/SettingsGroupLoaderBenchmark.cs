...
public class SettingsGroupLoaderBenchmark
{
    ...
    private void BuildLargeJson(int itemCount)
    {
        ...
        for (var i = 0; i < itemCount; i++)
        {
            ...
            var validator = type switch
            {
                "bool" => "\"x => true\"", "int" => "\"x => x >= 0\"", _ => "\"x => x != null\""
            };
            var key = "item-" + i.ToString("D4");
            items.Add("\"" + key + "\":{\"general\":\"" + type + "\",\"name\":\"Item " + i + "\",\"default-value\":" +
                      defaultVal + ",\"validator\":" + validator + "}");
        }
    ...
    public SettingsGroup LoadFromJson_Basic()
    {
        return _loader.LoadFromJson("group", _basicJson);
    }

    [Benchmark(Description = "LoadFromJson 100 items")]
    public SettingsGroup LoadFromJson_Large()
    {
        return _loader.LoadFromJson("large", _largeJson);
    }

    [Benchmark(Description = "LoadFromJson nested (2 groups, 3 items)")]
    public SettingsGroup LoadFromJson_Nested()
    {
        return _loader.LoadFromJson("root", _nestedJson);
    }

    [Benchmark(Description = "LoadSettingItem<int> with validator")]
    public SettingItem<int> LoadSettingItem_WithValidator()
    {
        return _loader.LoadSettingItem<int>("port", _itemWithValidator);
    }

    [Benchmark(Description = "LoadSettingItem<int> without validator")]
    public SettingItem<int> LoadSettingItem_WithoutValidator()
    {
        return _loader.LoadSettingItem<int>("port", _itemWithoutValidator);
    }
}